...
function validateConfig(): EidolonConfig {
  ...
  const capabilitiesStr =
    ...
    "trading,token-launch,research,x402,erc8004";
  ...
  return {
    bankr: {
      ...
      llmBaseUrl: process.env.BANKR_LLM_BASE_URL,
      agentBaseUrl: process.env.BANKR_AGENT_BASE_URL,
    },
    treasury: {
      ...
      tokens: process.env.TREASURY_TOKENS
        ? process.env.TREASURY_TOKENS.split(",").map((s: string) => s.trim())
        : undefined,
    },
    erc8004: {
    ...
    x402: {
      ...
      paymentAddress:
        process.env.X402_PAYMENT_ADDRESS || process.env.TREASURY_WALLET!,
      pricing: {
        "/signals/price/:token": {
...
async function main() {
  ...
  try {
    ...
    orchestrator.on("log", (msg: string) =>
      logger.info(msg.replace(/\[Eidolon\]\s?/, "")),
    );
    orchestrator.on("alert", (msg: string) => logger.warn(msg));
    orchestrator.on("error", (msg: string) => logger.error(msg));
    orchestrator.on("trade", (data: any) =>
      logger.info(`Trade: ${JSON.stringify(data)}`),
    );
    orchestrator.on("payment", (data: any) =>
      logger.info(`Payment received from ${data.clientId}: $${data.amount}`),
    );
    orchestrator.on("launched", (data: any) =>
      logger.info(`Token launched: ${data.symbol} at ${data.address}`),
    );
